...
## Official placement letter (Stage 2 — PDF to organisation)

The PDF generated by `backend/services/pdfService.js` uses a **HOD** or **admin** signatory block depending on who last approved the placement (`internship_placements.reviewed_by` → user role).

- **Templates and optional images**: `backend/services/officialPlacementLetterSignature.js`
  - HOD: title `Head of Department`, department line = the student’s department, optional image `backend/public/signatures/official-placement-hod.png`
  - Admin: registrar-style lines, optional image `backend/public/signatures/official-placement-admin.png`
- **Fallback printed names** (optional env, used when the approver profile has no first/last name):
  - `OFFICIAL_PLACEMENT_HOD_SIGNATORY_NAME`
  - `OFFICIAL_PLACEMENT_ADMIN_SIGNATORY_NAME`  
  When the approver exists and has a first and last name, that name is shown on the signature line; otherwise the env value or the bracket placeholder in code is used.
