...
// InternshipPlacement model helpers
const InternshipPlacement = {
  async findOne(where) {
    let query = supabase.from('internship_placements').select('*');
    if (where.id) query = query.eq('id', where.id);
    if (where.studentId) query = query.eq('student_id', where.studentId);
    if (where.generalRequestId) query = query.eq('general_request_id', where.generalRequestId);
  ...
  async findAll(options = {}) {
    ...
    const { data, error } = await query;
    if (error) throw error;
    return data.map(mapPlacementFromSupabase);
  },
  ...
  async create(placementData) {
    const { v4: uuidv4 } = require('uuid');
    const mapped = mapPlacementToSupabase({ ...placementData, id: placementData.id || uuidv4() });
    const { data, error } = await supabase
...
const EvaluationToken = {
  ...
  async findByPlacement(placementId) {
    ...
    if (error) throw error;
    return data.map(mapEvalTokenFromSupabase);
  },
...
const EmailLog = {
  ...
  async findAll(options = {}) {
    ...
    const { data, error } = await query;
    if (error) throw error;
    return data.map(mapEmailLogFromSupabase);
  },
